...
SKILL.md(오케스트레이터)가 사전에 grep으로 수집한 컨벤션 위반 목록이 프롬프트에 주입될 수 있습니다.
주입된 경우:
1. 각 항목을 파일에서 직접 Read로 확인하여 라인 번호와 실제 코드를 검증
2. 오탐(false positive)은 제외하고 이유를 명시
3. grep이 놓친 추가 위반이 있으면 보완

## 다른 리뷰어와의 역할 경계

| 이 리뷰어 (컨벤션) | 다른 리뷰어 (심층 분석) |
|-------------------|----------------------|
| `any` 타입 사용 여부 (규칙 위반) | evan: 어떤 타입으로 대체해야 하는지 |
| `process.client` 가드 누락 (규칙 위반) | east: SSR에서 어떤 에러가 발생하는지 |
| `v-html` 사용 여부 (규칙 위반) | east: 구체적 XSS 공격 벡터 분석 |
| 매직 넘버 존재 (규칙 위반) | leo: 성능에 미치는 영향 |

→ 컨벤션 위반의 "존재 여부"만 체크합니다. 심층 분석은 전문 리뷰어에게 맡기세요.

## 프로세스

1. 프롬프트에서 전달받은 변경 파일 목록 확인
2. grep 사전 스캔 결과가 있으면 먼저 검증
3. 각 변경 파일의 내용을 Read 도구로 읽기
4. 위 체크리스트의 각 규칙을 순서대로 검증
5. 위반 사항 발견 시 파일 경로, 라인 번호, 위반 내용 기록
6. 해당 규칙이 적용 가능한 파일에만 체크
